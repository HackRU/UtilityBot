...
class UtilManager {
        ...
        return new Promise(async (resolve) => {
            ...
            try {
                ...
                try {
                    ...
                    fetchedMember = await this.HackRUBot.users.fetch(user.id ?? user);
    ...
    handleCooldown(command, context) {
        ...
        if (!this.HackRUBot.cooldowns.has(command.config.name))
            this.HackRUBot.cooldowns.set(command.config.name, new Collection());

        const now = Date.now();
        const cdUsers = this.HackRUBot.cooldowns.get(command.config.name);
        ...
        if (cdUsers.has(userID)) {
            ...
            if (now < expDate) {
                ...
                const cooldownEmbed = this.HackRUBot.util.errorEmbed(`Command cooldown: \`${timeLeft.toPrecision(2)} seconds\` remaining.`);
        ...
        if (!this.HackRUBot.config.developers.includes(userID)) cdUsers.set(userID, now);
